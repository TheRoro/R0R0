...
client.on('message', async message => {
    ...
    else if (command === "cat" || command === "neko" || command === "gato" || command === "🐈" || command === 'c') {
        await client.commands.get('cat').execute(message, args)
    }
    else if (command === "rick" || command === "morty" || command === "geez" || command === 'r') {
        await client.commands.get('rick_and_morty').execute(message, args)
    }
    else if (command === "pokemon" || command === "poke" || command === "pkmn" || command === "pk" || command === 'p') {
        await client.commands.get('pokemon').execute(message, args)
    }
    else if (command === "command" || command === "commands" || command === "commandList") {
